...
Driver::Driver(StringRef DriverExecutable, StringRef TargetTriple,
    ...
    : Driver(llvm::ToolContext::buildRoot(nullptr).build(
                 ArrayRef<const char *>{DriverExecutable.data()}),
...
Driver::Driver(llvm::ToolContext CallingTool, StringRef TargetTriple,
    ...
    : Diags(Diags), VFS(std::move(VFS)), Mode(GCCMode),
      ...
      ModulesModeCXX20(false), DriverToolContext(CallingTool),
  ...
  Name = std::string(llvm::sys::path::filename(DriverToolContext.getPath()));
  Dir = std::string(llvm::sys::path::parent_path(DriverToolContext.getPath()));
  ...
  ResourceDir = GetResourcesPath(DriverToolContext.getPath());
}
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  auto DriverMode =
      getDriverMode(DriverToolContext.getToolName(), ArgList.slice(1));
  if (!DriverMode.empty())

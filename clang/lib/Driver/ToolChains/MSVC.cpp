...
void visualstudio::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  llvm::ToolContext LinkerTool =
      D.getToolContext().build(ArrayRef{Args.MakeArgString(linkPath)});
  if (D.InProcess && !D.CCGenDiagnostics && Linker == "lld-link") {
    auto EC = LinkerTool.getCallableTool();
    if (EC) {
      // Invoke LLD directly in this process.
      Job = std::make_unique<CC1Command>(
          JA, *this, ResponseFileSupport::AtFileUTF8(), LinkerTool, *EC,
          CmdArgs, Inputs, Output);

      // Signify that the LLD tool supports the -disable-free flag, and by
      // default doesn't free memory or clean resources, it lets the OS do it
      // on process exit. Always free resources when running LLD in-process,
      // otherwise diagnostics will not be generated by Clang upon a crash.
      Job->DisableFree.emplace(false);

      // Adjust the command-line for -disable-free. If the user has decided to
      // force the flag, don't do anything.
      bool DisableFreeOverriden = llvm::any_of(
          C.getArgs().filtered(options::OPT_Xclang), [](auto *Arg) {
      ...
      if (!DisableFreeOverriden) {
        Job->PostBuildJobs = [](Command *Cmd) {
          ...
          Args.push_back(*Cmd->DisableFree ? "-disable-free"
                                           : "-no-disable-free");
        };
      }
    }
  ...
  if (!Job) {
    // Execute the job out-of-process. MSVC's link.exe expects UTF-16 response
    // files, so use AtFileUTF16() for the out-of-process path.
    Job =
        std::make_unique<Command>(JA, *this, ResponseFileSupport::AtFileUTF16(),
                                  LinkerTool, CmdArgs, Inputs, Output);
  }

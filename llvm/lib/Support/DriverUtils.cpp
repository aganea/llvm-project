...
struct ProcessState {
  ...
  /// Tracks module names that have already been attempted for dynamic loading,
  /// to avoid redundant loads when a module fails to load or is not found.
  llvm::DenseSet<StringRef> SeenModules;
...
// Forward declaration -- implemented in DynamicLibrary.inc (Windows only).
// Loads an external LLVM tool module and returns its main function along with
// the list of tool names it exports.
ErrorOr<llvm::ToolMainFn> loadCallableModule(StringRef ModuleName,
                                             std::vector<StringRef> &ToolNames);
...
ToolContext::getCallableTool(ArrayRef<const char *> &Args) const {
  ...
  // Look for registered tools first.
  for (auto &RegisteredTool : State.RegisteredTools) {
    if (Is(RegisteredTool.ToolName, ToolName))
      return RegisteredTool;
  }

  // If the tool is not found, attempt to look it up in LLVM's current bin/
  // folder. If the module is loaded, it'll also be registered in the vector
  // above.
  StringRef ToolStem = sys::path::stem(ToolName);
  if (!State.SeenModules.contains(ToolStem)) {
    std::vector<StringRef> Names;
    auto ModuleFn = loadCallableModule(ToolName, Names);
    if (std::error_code EC = ModuleFn.getError()) {
      // Only mark the module as "seen" if the error is NOT "file not found".
      // When the file doesn't exist yet (e.g. it hasn't been built), we want
      // to allow a later retry once the binary becomes available. For other
      // errors (e.g. the file exists but failed to load), we mark it to
      // prevent repeated failing load attempts.
      if (EC != std::errc::no_such_file_or_directory)
        State.SeenModules.insert(ToolStem);
      return EC;
    }

    // Module loaded successfully -- mark it so we don't reload.
    State.SeenModules.insert(ToolStem);

    for (StringRef Name : Names)
      State.RegisterDriverTool({Name, ModuleFn.get()});

    // Return the last inserted tool.
    return State.RegisteredTools.back();
  }
...
int LLVMDriverCallMain(int Argc, char **Argv, StringRef Name,
                       ToolMainFn MainFn, bool NeedsPOSIXUtilitySignalHandling) {
  InitLLVM X(Argc, Argv, /*InstallPipeSignalExitHandler=*/ true, NeedsPOSIXUtilitySignalHandling);
...
//===----------------------------------------------------------------------===//
// Windows-only: in-process module loading support
//===----------------------------------------------------------------------===//

#if defined(_WIN32)
// Implemented in InitLLVM.cpp.
void OnlyInitializeCRT(void *InitEvent, void *ExitEvent);

// In C:\Program Files\Microsoft Visual
// Studio\{vs_version}\{variant}\VC\Tools\MSVC\{msvc_version}\crt\src\vcruntime\tlsdyn.cpp
extern "C" void __dyn_tls_init(void *, unsigned long, void *);
extern "C" void __dyn_tls_dtor(void *, unsigned long, void *);

/// Build a ToolSymbolTable for export from a dynamically loadable LLVM tool
/// binary. This is called by the getCallableTool() function generated by the
/// LLVM_DRIVER_IMPL_MAIN macro. The table captures the tool's main function
/// along with CRT initialization hooks needed for in-process loading.
ToolSymbolTable
LLVMDriverImplementTool(StringRef Name, ToolMainFn MainFn,
                        ToolSymbolTable::MainCRTStartupFn MainCRTFn) {
  ToolSymbolTable Tbl;
  Tbl.Tool = CallableTool{Name, MainFn};
  Tbl.CRTInit = &OnlyInitializeCRT;
  Tbl.MainCRT = MainCRTFn;
  Tbl.TLSInit = &__dyn_tls_init;
  Tbl.TLSDtor = &__dyn_tls_dtor;
  return Tbl;
}
#endif // _WIN32
